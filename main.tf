...
resource fortios_system_interface port {
  ...
  name          = var.name
  ip            = try(length(var.ip_addresses), 0) > 0 ? var.ip_addresses[0] : null
  vdom          = var.vdom
  autogenerated = "auto"
  allowaccess   = var.allowaccess
  secondary_ip  = try(length(var.ip_addresses), 0) > 1 ? "enable" : "disable"
  dynamic secondaryip {
    for_each = { for ip in try(slice(var.ip_addresses, 1, length(var.ip_addresses)), []) : ip => ip}
    content {
